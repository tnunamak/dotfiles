...
> **[PERSONA SCOPE NOTE 2026-08-19 — APPLICABILITY, NOT ACCURACY.]** The CLAIMS here are accurate prior art about *dev-tool and operator* surfaces (Linear, Stripe, GitHub, Vercel) and are unchanged. What changed is transfer: PDPP is now a **consumer** product per the product owner (see `product-design/consumer-products-do-show-raw-condition-lists-to-owners-...md`), and this entry was extracted from a PDPP console-redesign note. The two density claims are the ones to re-examine before applying — the 14px body size and the "compress whitespace, 4/8/12px rather than 16–24px" rule are both explicitly justified by *dense operator/dev-tool* context, and consumer products generally run larger type and more whitespace. Treat them as evidence about power-tool surfaces, not as a PDPP recommendation. **Persona-independent and safe to apply as-is:** tabular/lining numerals on all data, a single named spacing scale, the small intentional radius scale, elevation as surface-fill-step + hairline border, no zebra striping, and master-detail as the drill-down shell. Note the highest-leverage item in this entry (`tabular-nums lining-nums`) and its biggest anti-pattern (ad-hoc spacing with no named scale) are both craft rules that hold for any audience.
