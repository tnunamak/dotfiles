...
- **A dereferenceable MyTerms agreement identifier exists TODAY.** A protocol with a dedicated agreement-reference field can reuse a rostered term rather than minting a competing identifier. A purpose field must not carry it: purpose and agreement are separate semantic objects. [cc-agreements] [cc-p2b1]
...
- IEEE offers the IEEE 7012-2025 full text at no cost through the IEEE GET Program, which requires accepting GET terms. Xplore returned HTTP 418 in this environment, so the full text was not independently inspected here. The scope statement and public summary are readable on the standards.ieee.org project page. [ieee-7012-page]
...
On MyTerms compatibility, the genuinely useful finding is that **a dereferenceable rostered identifier exists today** (`customercommons.org/agreements/p2b1/0.9/`). That turns "reference rather than invent" from a principle into an executable option once a protocol has a dedicated agreement-reference field. The corresponding risk is equally concrete: minting a closed bespoke vocabulary for terms a roster already names makes two records of the same real-world agreement machine-incomparable. Reusing the identifier does not justify routing it through a purpose field; the same research found purpose and agreement to be separate objects.

Temper the enthusiasm on two axes. IEEE 7012 defines no OAuth-family wire format and is protocol-agnostic, so "compatible" is currently true at the terms-vocabulary layer and unproven at the dual-record layer. IEEE expects both parties to hold a record and explicitly puts party-to-party negotiation outside scope; the separate DPV community extension is what describes counter-offers. Most OAuth-family designs define only the authorization server's copy and issue after consent. Adoption is near zero: a forming alliance and a small pilot rather than a production flow. Reference it as a hedge; do not build a dependency on it yet.
