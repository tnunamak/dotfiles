...
> **[PERSONA SCOPE NOTE 2026-08-19.]** PDPP is now a **consumer** product per the product owner (the earlier technical-operator reading is void — see `product-design/consumer-products-do-show-raw-condition-lists-to-owners-...md`). Two clauses below are scoped to a self-hoster and should be read accordingly. **Point (5)** presumes the user has operator prerequisites (an instance-level encryption key, a deployment secret) to satisfy; a consumer on a hosted install has none, so that move becomes vestigial rather than wrong — the *rule* (disclose blocking prerequisites before the user spends effort) survives; the *example class* does not apply. **The closing contract** says readiness should be stated "in the operator's own language"; under the consumer requirement read this as **the owner's own language**. The substance of that clause is unchanged and is in fact the consumer-safe reading: name the exact missing setting in the reader's vocabulary rather than in internal tokens. Everything else here (staged journey, product-observed status, identity echo, access review as a product surface, distinguished credential types) is persona-neutral, and point (6) — access review as a product surface, not a raw audit log — is actively pro-consumer.
