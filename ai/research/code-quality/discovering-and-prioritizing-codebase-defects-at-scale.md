...
- **Test quality — StrykerJS mutation testing** is the sole EXPENSIVE / sample-and-triage tool and the highest-value blind spot. Current StrykerJS defaults `coverageAnalysis` to `perTest` for runners that supply the required coverage detail; `--incremental` can reuse compatible prior results. Runner support remains load-bearing: the command runner reports no test details, weakening test-change detection and selective reuse for PDPP's current `node:test` setup. A *survived* mutant means a line is covered yet the suite fails to catch an injected defect — the only signal that measures assertion strength, which line coverage cannot. Default thresholds `{high: 80, low: 60, break: null}`. FP risk MED–HIGH from equivalent mutants (no test can ever kill them); requires human/judge triage. [stryker-docs] [stryker-incremental]
...
Accessed: 2026-08-11
Quote: current default `coverageAnalysis: "perTest"`; the documentation records that the default changed from `"off"` to `"perTest"`. Per-test coverage lets Stryker run only tests covering a mutant, while unsupported/static cases fall back to broader execution. Threshold defaults remain `{ high: 80, low: 60, break: null }`; mutation score = detected/valid×100; a survived covered mutant nominates a test gap.
