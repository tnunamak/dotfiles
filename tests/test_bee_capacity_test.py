...
class CandidateServiceTests(unittest.TestCase):
    ...
    def test_candidate_is_active_with_mainpid_and_cgroup_before_proof(self):
        runner = lambda command, timeout: done(command, stdout=b"ActiveState=active\nMainPID=123\nControlGroup=/candidate\nNTasks=2\n")
        self.assertTrue(m.unit_is_running(runner, "systemctl", "candidate.service", 1))
...
class ReceiptAndOwnerTests(unittest.TestCase):
    def make_owner(self, receipt_mutate=None, show=b"ActiveState=inactive\nNTasks=0\n", verify=True):
        ...
        def runner(command, timeout):
            ...
            if "show" in command and command[3] == "candidate.service": return done(command, stdout=show)
            return done(command)
...
class ControllerTests(unittest.TestCase):
    def test_controller_never_acknowledges_or_restores(self):
        directory = tempfile.TemporaryDirectory(); self.addCleanup(directory.cleanup)
